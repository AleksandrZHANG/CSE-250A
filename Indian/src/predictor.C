// Touranament Predictor (gshare+bimodal) 
// cost=gShare+bimodal+choice=2^13x2+ 2^12x2+2^12x2=32768<33024=(32K+256)(bit)
// gShare 
...
void init_predictor () 
{ 
  for(int i=0;i<gs_PHT_SIZE;i++)//init gShare
    gs_PHT[i] = 0; 
  gs_HISTORY = 0; 
...
{ 
  ...
  { 
    if(gsPredict == outcome && CHOICE[gs_HISTORY & choice_MASK]<3)
      CHOICE[gs_HISTORY & choice_MASK]++;	//train chooser to choose GLOBAL 
    else if(biPredict == outcome && CHOICE[gs_HISTORY & choice_MASK] > 0)
      CHOICE[gs_HISTORY & choice_MASK]--;	//train chooser to choose LOCAL
  }
